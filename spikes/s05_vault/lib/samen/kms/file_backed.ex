defmodule Samen.Kms.FileBacked do
  ...
  is the adapter the S0.5 red-path tests run against, because it can simulate
  ...
  `:s05_vault, :kms_key_dir`. NEVER under the Postgres data dir or the repo.
  """
  @spec key_dir() :: String.t()
  def key_dir do
    Application.get_env(:s05_vault, :kms_key_dir) ||
      Path.join(System.tmp_dir!(), "samen_s05_keystore")
  end
  ...
  defp effective_dir do
    if outage?() do
      ...
      Path.join(System.tmp_dir!(), "samen_s05_keystore__UNREACHABLE__#{:erlang.unique_integer()}")
    else
      key_dir()
    end
  end
  ...
  defp eff_dek_path(subject_id), do: Path.join([effective_dir(), "subjects", "#{subject_id}.dek"])
  ...
  defp read_tombstone!(subject_id) do
    ...
    # string; normalize back to the behaviour's attestation shape so callers
    # always see `state: :shredded` (atom) and a DateTime, not strings.
    %{
      map
