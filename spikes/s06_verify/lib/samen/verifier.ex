defmodule Samen.Verifier do
  ...
  (e.g. `Mix.Tasks.Samen.Verify.CatalogParity`) calls `run_check/2` with a list
  of violations; the harness prints them and exits non-zero if the list is
  non-empty.

  ## Exit behaviour (fail-closed guarantee)

  `halt_if_violations/1` calls `:erlang.halt(1)` directly (not `System.stop/1`)
