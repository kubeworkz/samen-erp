defmodule Samen.Transformers.AbbrevStorage do
  ...
  idiom. Reused verbatim from the S0.2 spike (see spikes/s02_transformer).
  ...
  S0.4 relies on this so that `Ash.Resource.Info` introspection (which the
  Samen.Migration diff reads) yields the prefixed storage names that the catalog
  rows must record — proving catalog contents match introspection.
  """
