defmodule Driftwood.SchemaDictDriftTest do
  @moduledoc """
  In-suite tripwire for `driftwood/ci.sh` step 1b ("schema.dict.json drift
  ...
  The B1 movement ledger (`SubscriptionEvent`) landed in Driftwood's Billing
  AND Operator domains (`dpv_subscription_event`, `fbv_subscription_event`)
  but `schema.dict.json` was not regenerated — the same omission that turned
  the demo gate red, propagated into this vertical. This test makes that class
  of drift fail loudly in `mix test`.
  """
  ...
  @mov_tables ~w(dpv_subscription_event fbv_subscription_event)
  ...
  test "GREEN: committed dict is byte-identical to a fresh dump" do
    ...
    assert committed_json == fresh_json,
           "driftwood/schema.dict.json is stale — run " <>
             "`MIX_ENV=test mix samen.catalog.dump --output schema.dict.json` and commit."
  end

  test "committed dict carries the B1 mov ledger blocks" do
  ...
  test "RED PATH: a dict missing a mov ledger block does NOT match the committed file" do
    mutated =
    ...
    refute (Jason.encode!(mutated, pretty: true) <> "\n") == File.read!(@committed_path),
           "drift check must reject a dict missing the mov ledger blocks"
  end
end
