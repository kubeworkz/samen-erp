defmodule Driftwood.Repo.Migrations.MigrateActivityToTask do
  @moduledoc """
  ADR-041 §5 (ruling M5) — the destructive CRM `Activity` → canonical Work-scope
  `Task` migration for the driftwood host (`fac_activity` → `dwt_task`). See
  ...
  Freight note: `fac_activity` was the CheckCall event stream (re-identified via
  `Driftwood.Context`); post-migration CheckCall aliases `Driftwood.Work.Task`.
  Contract phase, PITR-covered.
  """
  use Samen.Migration, phase: :contract

  def up do
    if table_exists?("fac_activity") do
      execute(copy_sql("fac", "dwt"))
    end

    execute("DROP TABLE IF EXISTS fac_activity")
    execute("DELETE FROM fld_field WHERE fld_table_name = 'fac_activity'")
    execute("DELETE FROM tam_table WHERE tam_table_name = 'fac_activity'")
  end
