defmodule Driftwood.Repo.Migrations.AiAssistantAssistantConversation do
  @moduledoc """
  `ai_assistant` / `ai_assistant_conversation` — the OpenClaw-lite assistant
  (P1) — the org-scoped assistant definition (`ast`, `ai_assistant`) + the
  vault-routed conversation thread under it (`asc`,
  `ai_assistant_conversation`). Driftwood mirror of
  `samen_core/priv/test_repo/migrations/20260925100000_ai_assistant_assistant_conversation.exs`
  (and `samen_web/priv/repo/migrations/...`).

  Allocator-owned abbrevs (`samen.abbrev.reserve --host samen_core` — `ast` =
  `Samen.AI.Assistant`, `asc` = `Samen.AI.AssistantConversation`). The
  conversation's ONE text artifact (`pii_asc_transcript`) is vault-routed
  precisely like `ai_agent_run.pii_arn_transcript`: the domain column holds a
  `vt_*` token; the ciphertext lives in `pii_vault` under the row's OWN DEK.
  `catalog_sync/1` keeps `mix samen.verify.catalog_parity` green in the SAME
  transaction as the DDL (ADR-004).
  """
  use Samen.Migration
