defmodule Driftwood.Repo.Migrations.InstallAshMoneyExtension do
  @moduledoc """
  ADR-036 D1/D7 (H1); ADR-037 §5.2 (AshMoney ADOPT). Installs the Postgres
  `money_with_currency` composite type + the `+`/`-`/`*`/`<`/`<=`/`>`/`>=`/`sum`/
  `min`/`max` SQL operators `AshMoney.AshPostgresExtension` (install versions 0-5)
  defines — the storage shape the H1 Money migration (`20260721030000_h1_money_migration`) needs before it can
  add the `*_value`/`*_unit_amount` composite columns.

  Content generated via `AshMoney.AshPostgresExtension.install/1` for versions 0-5
  (the same content `mix ash_postgres.generate_migrations` would splice into an
  auto-generated extension-install migration — reproduced by hand here because
  this host's migrations are hand-authored, matching the existing
  `AddAshFunctionsExtension`/`ash-functions` precedent in this migrations
  directory, not `mix ash_postgres.generate_migrations` codegen).

  Pure extension DDL — no catalog-tracked resource column changes here, so no
  `catalog_sync/1` (mirrors the ash-functions extension migration).
  """
