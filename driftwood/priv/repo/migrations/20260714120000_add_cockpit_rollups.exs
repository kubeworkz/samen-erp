defmodule Driftwood.Repo.Migrations.AddCockpitRollups do
  @moduledoc """
  ...
  SAME transaction (ADR-004 catalog-in-tx).
    ...
    * `mrr_revenue_rollup` — grain (org_id, period_month, mov_kind) → sum/count,
      ...
      (`dpv_subscription_event` — the SaaS's own MRR, the cockpit semantic).
    * `paf_product_event_rollup` — the funnel/retention seed rollup over the `fae`
  ...
  @tables [
    {"Driftwood.Aggregate.RevenueRollup", "mrr_revenue_rollup",
     [
       {"mrr_id", "id", "UUID"},
    ...
    {"Driftwood.Analytics.ProductEventRollup", "paf_product_event_rollup",
     [
       {"paf_id", "id", "UUID"},
