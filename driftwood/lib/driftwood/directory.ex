defmodule Driftwood.Directory do
  @moduledoc """
  The Driftwood tenant DIRECTORY (ADR-013 §4.2 / §7) — the `{tenant_org_id, name}` list the
  framework workspace switcher + the default-org / name resolution read on a plain tenant/shared
  mount (CRM/Billing/Support/Marketing/Chat), which cannot see the operator namespace itself.

  Each row is an operator-side ACCOUNT `Identity.Org` (Bridge-B): its `slug` carries the
  tenant_org_id back-reference, its `name` is the brokerage's display name. This is the SAME
  book of business `Samen.Web.Operator.Reads.accounts/3` assembles, projected to the minimal
  non-PII directory shape the switcher needs. Wired onto the mounts via the
  `org_directory: {Driftwood.Directory, :orgs, []}` label in `DriftwoodWeb.Router`.
