defmodule Demo.Repo.Migrations.WidenSessionInsertedAtUsec do
  @moduledoc """
  ADR-035 §4.3 A4 (T104) — widen the demo host's `Identity.Session` creation
  timestamp (`ses_inserted_at`) from second precision (`:utc_datetime`) to
  ...
  adapted to demo's `ses` abbrev. Existing rows widen losslessly; no
  ...
  def up do
    alter table(:ses_session) do
      modify(:ses_inserted_at, :utc_datetime_usec, from: :utc_datetime, null: false)
    end
  end

  def down do
    alter table(:ses_session) do
      modify(:ses_inserted_at, :utc_datetime, from: :utc_datetime_usec, null: false)
    end
  end
end
