defmodule Samen.WebTest.Repo.Migrations.ExpandAddPersonEngagementNote do
  @moduledoc """
  UXD-04 follow-up (W5): EXPAND phase — additively add a nullable operational column
  `swp_engagement_note` to `swp_person`, and record the bake-window meta row so a
  paired CONTRACT migration could gate on it.

  At HEAD this repo (like `pawchart`) carried zero `:expand`-tagged migrations in its
  ...
  ## Why this is a valid expand
    ...
    * `add_nullable_column/3` emits `ALTER TABLE ... ADD COLUMN swp_engagement_note
  ...
  `swp_engagement_note` is an operational shadow column (not an Ash attribute), so it
  is added to `:samen_web, :catalog_parity_allow_list` in config rather than
  catalogued — the same pattern `driftwood`/`demo` already use.

  `samen_web`'s `priv/repo/migrations` has no `priv/drills/` directory (checked before
  ...
  @change_key "samen_web_person_engagement_note"

  def change do
    expand_setup(change_key: @change_key)
    add_nullable_column(:swp_person, :swp_engagement_note, :text, default: "")
  end
end
