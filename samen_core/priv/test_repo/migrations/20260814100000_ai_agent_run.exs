defmodule SamenCore.TestRepo.Migrations.AiAgentRun do
  @moduledoc """
  `ai_agent_run` — the durable agent-run cursor (ADR-047 §4.1/§6, batch A1).
  Allocator-owned abbrev `arn` (`samen.abbrev.reserve --host samen_core --owner
  Samen.AI.Agent.Run --abbrev arn`). TOKEN-ONLY at rest (ADR-047 §6): ids, enums, counts,
  timestamps — no prompt/completion text column exists on this table (the vault-routed
  transcript is batch A2, inside the DEK envelope). `catalog_sync/1` writes the
  `tam_table`/`fld_field` rows in the SAME transaction as the DDL (the `ai_prompt`
