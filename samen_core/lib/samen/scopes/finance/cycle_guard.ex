defmodule Samen.Scopes.Finance.CycleGuard do
  @moduledoc """
  Refuses a `parent_id` write on the self-referential `Account` CoA tree
  (WS-ERP E1; ADR-049 §2 — the chart of accounts is a tree, and an account can
  never be made its own transitive ancestor) and bounds the walk depth so a
  pathological/corrupted chain cannot hang a write.

  Same-CycleGuard-lineage as `Samen.Scopes.Work.CycleGuard` (ADR-041 §3.4) —
  ...
  Runs only when `parent_id` is being changed. `nil` (detaching from a parent)
  is always a no-op. A legal N-level CoA is accepted; a cycle (including the
  self-parent degenerate case) is refused with a positive control retained by
  the caller's test (a legal re-parent still succeeds).
  """
  ...
  defp cycle_error(changeset) do
    Ash.Changeset.add_error(changeset,
      field: :parent_id,
      message: "an account cannot be its own ancestor (cycle refused)"
    )
  end
