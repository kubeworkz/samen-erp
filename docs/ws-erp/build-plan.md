...
(22 = the same Fleet/J-cockpit class).## Phase E8 — Reports, BI, surfaces, host proof, workstream gate · mixed — **DONE (2026-09-14)**
...
Status: **DONE, execution-verified (2026-09-14).**

- **Reports (samen_core):** `Budget`/`BudgetLine` in the Finance scope (the `lines`
  argument shape, admin-gated writes, dup-account refusal) + the `budget_vs_actual/2`
  pure read over posted GL facts; `Samen.E8Rollups` registers **TB / WIP / headcount**
  as `source: :domain` `Samen.Rollup.Spec`s over the append-only GL / stock / employment
  ledgers (ADR-018's generalization — no new mechanism, the existing refresh machinery
  recomputes them); `Samen.E8Aggregate` is the token-blind operator projection with
  CohortSpec floors (`%Suppressed{}` under k). Suite: `samen_core/test/e8_reports_test.exs`
  (10 green, red paths paired with positive controls).
- **Surfaces (samen_web):** `Samen.Web.Erp` — the SIX-surface closed allowlist (accounts,
  journal, ap_invoices, stock, purchase_orders, work_orders) whose registry IS the
  boundary; `Samen.Web.Erp.SurfaceLive` — ONE generic read-only LiveView; the
  `samen_erp_routes/2` router macro mounts all six with ONE authored line (org-scoped,
  planes-gated, no vault field is renderable — the registry declares only non-PII
  columns and the walkthrough asserts every declared column exists on the resolved
  resource).
- **The host proof (`samenerp/`):** generated by `mix samen.gen.app` (prefix `er`), then
  ONE domain module (`Samenerp.Erp`) mounts Finance + Inventory AS-IS over the host
  namespace — the E1–E7 resource set with the cross-scope bridges re-pointed to the
  host's own modules (`finance:` → `Samenerp.Finance`, `billing:` → the host's REAL
  `Samenerp.Billing.Invoice`); ~46 host abbrevs reserved via the sanctioned allocator;
  the fixture DDL merged into ONE host migration. The walkthrough
  (`samenerp/test/erp_walkthrough_test.exs`, 5 blocks) replays E3 ledger facts, the E4
  gated chokepoint (stock + GL in ONE transaction), the E5 bridge emitting into the REAL
  Billing invoice, the E6 BOM snapshot, and the surfaces registry resolution. Authored
  LOC for the whole ERP mount: one domain module + one router line.
- **The host gate:** `samenerp/ci.sh` runs the FULL 19-step verifier suite
  (catalog/prefixes/pii/migrations/tnt/same_org_fk/erasure/aggregate_privacy/api_contract
  + tests + the vault anti-tautology probe). Its F3.5 sweep caught a REAL blueprint-level
  gap — `BudgetLine`/`StockLevel`/`GoodsReceipt` carried no `SameOrgFk` guard in the
  SHARED blueprints (the fixture test resources had their own) — fixed at blueprint
  level, samen_core suites re-run green.
- **Workstream re-gate:** sabotage patches **302–308** (R1–R5 + the E7 roster plane)
  re-flipped from the harness — `SABOTAGE HARNESS: ALL PASSED (7 of 308 — FILTERED:
  range=302-308)`, byte-exact restores; the E7 masking trios re-run green (38 + 12);
  root `ci.sh` now runs **five** gates (samen_web, demo, driftwood, pawchart,
  **samenerp**) and ends `ROOT CI: ALL PASSED`.
