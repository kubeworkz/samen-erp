...
## State after WS-ERP (2026-09-14) — the ERP base system

WS-ERP (E0–E8, 2026-09-12 → 2026-09-14) landed the seven-component ERP core as REUSABLE kernel
scopes + the `samenerp` host proof (design: `docs/ws-erp/design.md`; ADR-049; full evidence:
`docs/claim-evidence.md` section P, per-phase status in `docs/ws-erp/build-plan.md`):

- **Finance** (CoA tree, event-sourced GL, R1 double-entry red suite) · **AP/AR** documents with the
  ADR-040 Gate and source-anchor postings (R2) · **Inventory** (append-only stock ledger,
  derived-by-construction rollup, NegativeStock fail-closed, R3) · **Procurement** (the one-transaction
  Finance↔Inventory chokepoint + three-way match, R5) · the **Sales bridge** (Lead → SO → stock + REAL
  Billing invoice) · **Manufacturing** (BOM cycle refusal, frozen BOM snapshot, exactly-once
  consume/produce/log, R4) · **HR** (the first non-empty PII map — INV-1 masking + reveal gates + the
  mask-by-omission roster CSV).
- **E8 reports:** Budget/BudgetLine + `budget_vs_actual/2` pure read; TB/WIP/headcount as
  `source: :domain` Rollup specs (ADR-018 generalization — no new mechanism); the token-blind operator
  aggregate under CohortSpec k-anon floors; SIX mountable tenant surfaces behind a closed allowlist
  (`Samen.Web.Erp`) mounted by ONE router line over ONE generic read-only LiveView.
- **The host proof:** `samenerp/` (generated by `mix samen.gen.app`, prefix `er`) mounts ALL E1–E7 scopes
  AS-IS at ≈0 authored LOC — one domain module + one router line — and passes the FULL 19-step verifier
  gate; its F3.5 sweep caught and closed a real blueprint-level SameOrgFk gap (BudgetLine/StockLevel/
  GoodsReceipt). Root `ci.sh` now runs FIVE gates and ends `ROOT CI: ALL PASSED`.
- **Adversarial posture:** sabotage patches **302–308** committed (one per phase, re-flipped at the E8
  re-gate); masking trios re-run green; every R-test pairs denial with a positive control.

**Honest scope (design §7 — binding):** payroll/tax engines, multi-currency, lot costing, scheduling,
accruals, forecasting, depreciation are adapter-or-P2/P3 and were REFUSED into that list — the shipped
thing is the ledger + document spine, fail-honest about the rest.

**Next-workstream candidates (from this workstream):** a reference payroll adapter (the behaviour has no
engine), lot/serial costing on the stock ledger, the P2 AR-side customer statement/aging reads, and the
operator-plane ERP cockpit (the six surfaces are tenant-plane today).
