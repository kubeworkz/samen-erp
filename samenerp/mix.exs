defmodule Samenerp.MixProject do
  use Mix.Project

  # Samenerp — a Samen vertical app scaffolded by `mix samen.gen.app` (T6.4 + WS-D D2/D3).
  # Shaped like demo/driftwood/pawchart: mounts the samen_core Billing scope AS-IS,
  # authors one vertical resource (Samenerp.Vertical.Record) with a pii_ scalar
  ...
  defp deps do
    [
      ...
      {:phoenix_html, "~> 4.1"},
      # Bandit: the HTTP adapter behind SamenerpWeb.Endpoint.
      {:bandit, "~> 1.0"},
      {:phoenix_pubsub, "~> 2.1"},
