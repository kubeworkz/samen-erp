...
(`Samenerp.Vertical.Record`, abbrev `rne`) with a scalar
`pii_rne_secret` vault field, defines a token-blind aggregate projection,
...
Then open http://localhost:4050 — the landing page links every inherited
...
`../samen_core/_build/dev/lib/samen_core/priv/abbrev_registry.json` (the global registry): Billing scope
(`erc/ers/erl/erp/eri/ery/eru/ere`), the
authored resource (`rne`), the aggregate plane (`era`), the
Primitives mount (`ent/enp/efl/esh/ewh/eff`)
and the operator namespace (`eoo…/epc…/eqk…` — the
