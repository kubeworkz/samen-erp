defmodule Samenerp.RecordApiTest do
  @moduledoc """
  ...
  """
  use Samenerp.ApiCase, async: false
  ...
  describe "deny-by-default allowlist (AC-G4-3)" do
    setup do
      org = mk_org()
      secret = "SECRET-rne-#{System.unique_integer([:positive])}"
    ...
    test "a payload exposes ONLY the allowlisted catalog fields — the un-allowlisted vault field is ABSENT",
      ...
      # `type` is the catalog name, never the storage table (`rne_record`).
