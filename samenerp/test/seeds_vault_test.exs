defmodule Samenerp.SeedsVaultTest do
  @moduledoc """
  WS-D D4 red path (AC-G4-4): the seed idiom is vault-aware BY CONSTRUCTION.

  `Samenerp.Seeds.run/0` writes through `Samen.Factory` (the `SampleData` vault
  ...
  test "seeded 🔒 secrets are vault-routed — plaintext nowhere at rest" do
    ...
    assert org_id == Samenerp.Seeds.org_id()

    seeded_plaintexts =
      Samenerp.Seeds.records() |> Enum.map(fn {_n, _s, secret} -> secret end)
    ...
    %{rows: rows} =
      Ecto.Adapters.SQL.query!(
        Repo,
        "SELECT pii_rne_secret FROM rne_record " <>
          "WHERE rne_org_id = $1",
        [org_dumped]
      )
