defmodule Samenerp.Vertical do
  @moduledoc """
  Samenerp's vertical-authored domain — the "20%" this app writes itself.

  Ships ONE authored resource, `Samenerp.Vertical.Record`, a Tier-3 code
  composition (`use Samen.Resource`, abbrev `rne`) that inherits the ENTIRE
  ...
  field (`pii_rne_secret`) to exercise the vault/mask/reveal path end to end.
  ...
  resources do
    resource(Samenerp.Vertical.Record)
  end
end

defmodule Samenerp.Vertical.Record do
  @moduledoc """
  The authored vertical record (abbrev `rne`), with:

    * `pii_rne_secret` — a SCALAR pii_ vault field
      (`pii_attribute :secret, :string, vault: :pii_secret`). Masked `••••` by default;
      plaintext only via `:reveal_rne` under a distinct-party grant;
  ...
  # never storage names (`rne_name`, `pii_rne_secret`).
  #
