defmodule Samenerp.Aggregate do
  @moduledoc """
  Samenerp's token-blind aggregate plane (doc §control "Cross-tenant views run on
  ...
  resources do
    resource(Samenerp.Aggregate.RecordCountBySegment)
  end
end

defmodule Samenerp.Aggregate.RecordCountBySegment do
  @moduledoc """
  Cross-tenant RECORD COUNT by segment. A `use Samen.Aggregate.Resource` projection
  over the vault-excluded `era_record_count` summary table. Every column is bounded /
  ...
  use Samen.Aggregate.Resource,
    otp_app: :samenerp,
    domain: Samenerp.Aggregate,
    data_layer: AshPostgres.DataLayer,
    authorizers: [Ash.Policy.Authorizer],
    abbrev: "era"

  postgres do
    table("era_record_count")
    repo(Samenerp.Repo)
  end
