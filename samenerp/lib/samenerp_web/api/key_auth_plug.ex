defmodule SamenerpWeb.Api.KeyAuthPlug do
  @moduledoc """
  ...
  An api_key row (`Samenerp.Operator.ApiKey` — the operator Identity mount's
  ...
  defp build_actor(key_row) do
    ...
    case Ash.load(key_row, [membership: membership_query], authorize?: false) do
      {:ok, %{membership: %{} = mbr} = key_row} ->
        org_id = fetch(mbr, [:org_id, :eom_org_id])
        user_id = fetch(mbr, [:user_id, :eom_user_id])
