defmodule SamenerpWeb.Api.Router do
  @moduledoc """
  ...
  this pipeline (see `SamenerpWeb.Api.Endpoint`), so every route here lives under
  `/api/v1` (`/api/v1/records`, `/api/v1/records/:id`).

  ## Governed both ways

  Inbound requests carry a plane-bearing actor set by `SamenerpWeb.Api.KeyAuthPlug`
  ...
  the resource `json_api` block (Samenerp.Vertical.Record).
  """
  use AshJsonApi.Router,
    domains: [Samenerp.Vertical],
    prefix: "/api/v1"
end
