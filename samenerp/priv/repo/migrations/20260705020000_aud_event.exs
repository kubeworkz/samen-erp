defmodule Samenerp.Repo.Migrations.AudEvent do
  @moduledoc "T2.2: the aud_event append-only event/audit tier."
  use Ecto.Migration

  # The Postgres role the app CONNECTS as — the REVOKE UPDATE/DELETE target that makes
  # aud_event append-only for the app (a superuser/owner bypasses it, by design). Resolved at
  # MIGRATION TIME (ADR-045 §4.2, O4) — NEVER a hardcoded developer laptop role, which would
  # ship a `REVOKE ... FROM <someone's-laptop-role>` into every adopter's prod migration:
  #   1. the explicit `:aud_event_app_role` knob (what a real prod deploy sets — see the deploy
  #      runbook's Operator TODO), else
  #   2. the repo's CONFIGURED `:username` (the role this app actually connects as in dev/CI),
  #      else
  #   3. RAISE a named error — refusing to guess a role, since a REVOKE against the WRONG role
  #      silently leaves aud_event mutable for the real app role (an audit-integrity gap).
  defp app_role do
    case Application.get_env(:samenerp, :aud_event_app_role) do
      ...
      _ ->
        repo_username =
          :samenerp
          |> Application.get_env(Samenerp.Repo, [])
        ...
        case repo_username do
          ...
          _ ->
            raise """
            #{inspect(__MODULE__)}: cannot determine the Postgres app role for the aud_event
            REVOKE UPDATE, DELETE grant.

            Set it explicitly (the deploy runbook's `aud_event_app_role` step):

                config :samenerp, :aud_event_app_role, "your_app_db_role"

            It could not be derived from the repo's configured :username either. Refusing to
            emit an arbitrary role — a REVOKE against the wrong role would silently leave
            aud_event mutable for the real app role (an audit-integrity gap).
            """
        end
    end
  end
