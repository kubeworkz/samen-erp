# Anti-tautology probe — the pii_rne_secret VAULT PATH (T6.4 generated red path).
#
# Guarantee under probe: the record vault round-trip (test/record_vault_test.exs) asserts
# (a) the raw pii_rne_secret column holds an opaque `vt_` token, NOT the plaintext,
...
kms_key_dir =
  Path.join(System.tmp_dir!(), "samenerp_probe_kms_#{System.system_time(:nanosecond)}")
...
plaintext = "PROBE-rne-SECRET-985"
...
raw_column = fn ->
  %{rows: [[raw]]} =
    Ecto.Adapters.SQL.query!(
      Repo,
      "SELECT pii_rne_secret FROM rne_record WHERE rne_id = $1",
      [id_dumped]
    )
...
IO.puts("== anti-tautology probe: pii_rne_secret vault path ==")
...
Ecto.Adapters.SQL.query!(
  Repo,
  "UPDATE rne_record SET pii_rne_secret = $1 WHERE rne_id = $2",
  [plaintext, id_dumped]
)
...
IO.puts("recovered on revert. The rne_secret vault round-trip test is NON-vacuous.")
File.rm_rf!(kms_key_dir)
