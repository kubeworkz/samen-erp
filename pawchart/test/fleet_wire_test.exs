defmodule PawChart.FleetWireTest do
  @moduledoc """
  ...
  wire." `samen_fleet_routes(otp_app: :pawchart)` is mounted in
  `pawchart_web/router.ex` — pawchart does NOT need the operator plane for
  this (§9.3's own dependency note: `samen_fleet_routes/1` is a plain
  controller pipeline; the reporting side and the T157-blocked operator-plane
  viewing side are deliberately split).
  """
  ...
  test "GET /fleet/health returns a schema-valid, correctly-signed FleetReport (mode A)" do
    ...
    :ok = LocalCredential.put(:pawchart, %{kind: :shared_secret, secret: secret})
    ...
    conn =
      ...
      |> Ingress.health(otp_app: :pawchart)

    assert conn.status == 200
    payload = Jason.decode!(conn.resp_body)
    assert :ok = Samen.Fleet.Report.Schema.validate(payload)
    # This is the VETERINARY domain shape — a real vertical, not a fixture.
  ...
  test "zero config: no fleet credential configured -> 503, never a fabricated 200" do
    conn = conn(:get, "/fleet/health") |> Ingress.health(otp_app: :pawchart)
