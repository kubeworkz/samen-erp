defmodule PawChart.Repo.Migrations.MigrateActivityToTask do
  @moduledoc """
  ADR-041 §5 (ruling M5) — the destructive CRM `Activity` → canonical Work-scope
  `Task` migration for the pawchart host (`vce_activity` → `pwt_task`). See
  ...
  def up do
    if table_exists?("vce_activity") do
      execute(copy_sql("vce", "pwt"))
    end

    execute("DROP TABLE IF EXISTS vce_activity")
    execute("DELETE FROM fld_field WHERE fld_table_name = 'vce_activity'")
    execute("DELETE FROM tam_table WHERE tam_table_name = 'vce_activity'")
  end
