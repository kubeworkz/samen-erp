...
# PawChart — production COMPILE-TIME config (ADR-045 §2.1). `config/config.exs` ends with
...
# in `pawchart/lib/pawchart/application.ex`).
config :pawchart, auth_required?: true

# --- KMS keystore posture (ADR-045 §4.2, O5) ------------------------------------------------
# PawChart has no `--deploy` `runtime.exs`, so no `:kms_adapter` is selected here and the vault
